        ...
        private void GetCurrentModel(bool skinOnly)
        {
            ...
            if (!skinOnly)
            {
                    ...
                    if (outfit[i].BodyType == BodyType.Head)
                    {
                        ...
                        foreach (TGI tgi in headTgis)
                        {
                            ...
                            if (geom != null && geom.ShaderHash != (uint)SimShader.Phong)
                            {
                                ...
                                if ((currentFrame & outfit[i].gender) == 0 && frameModifier != null && frameModifier.Length >= 2)
                                {
                        ...
                        //    collar = geom;
                        //}
                    if (outfit[i].BodyType == BodyType.Shoes && outfit[i].LinkList != null && outfit[i].RegionMapIndex < outfit[i].LinkList.Length)
                    {
                ...
                for (int i = 0; i < outfit.Length; i++)
                {
                    ...
                    if (tgis.Length > 0)
                    {
                        RegionMap rmap = (outfit[i].LinkList != null && outfit[i].RegionMapIndex < outfit[i].LinkList.Length) ? 
                            FetchGameRMap(outfit[i].LinkList[outfit[i].RegionMapIndex], packNames[i], outfitIndex, ref errorList) : null;
                        rejected = true;
                        foreach (TGI tgi in tgis)
                        {
                            ...
                            if (geom != null)
                            {
                                ...
                                if (bumpTGI == null && geom.Shader.normalIndex >= 0 && geom.TGIList != null && geom.Shader.normalIndex < geom.TGIList.Length) bumpTGI = geom.TGIList[geom.Shader.normalIndex];
                                if (emissionTGI == null && geom.Shader.emissionIndex >= 0 && geom.TGIList != null && geom.Shader.emissionIndex < geom.TGIList.Length) emissionTGI = geom.TGIList[geom.Shader.emissionIndex];
                    ...
                    Image texture = (outfit[i].LinkList != null && outfit[i].TextureIndex < outfit[i].LinkList.Length) ?
                        FetchGameTexture(outfit[i].LinkList[outfit[i].TextureIndex], outfitIndex, ref errorList, false) : null;
                    Image shadow = (outfit[i].LinkList != null && outfit[i].ShadowIndex < outfit[i].LinkList.Length) ?
                        FetchGameImageFromRLE(outfit[i].LinkList[outfit[i].ShadowIndex], outfitIndex, ref errorList) : null;
                    Image specular = (outfit[i].LinkList != null && outfit[i].SpecularIndex < outfit[i].LinkList.Length) ?
                        FetchGameImageFromRLE(outfit[i].LinkList[outfit[i].SpecularIndex], true, outfitIndex, ref errorList, true) : null;
                    Image bumpmap = null;
